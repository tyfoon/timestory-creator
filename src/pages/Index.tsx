  ...
  const validateForm = (): boolean => {
    ...
    if (type === 'birthdate') {
      if (!birthDate.day || !birthDate.month || !birthDate.year) {
        setErrors({
          main: t('required') as string
        });
        return false;
      }
      if (birthDate.year < 1900 || birthDate.year > currentYear) {
        setErrors({
          main: t('yearRange') as string
        });
        return false;
      }
    } else {
      ...
      if (yearRange.endYear <= yearRange.startYear) {
        setErrors({
          main: t('endYearAfterStart') as string
        });
        return false;
      }
    }
  ...
  return <div className="min-h-screen bg-gradient-hero">
      ...
      <section className="relative pt-28 pb-16 px-4 overflow-hidden">
        ...
        <div className="container mx-auto max-w-xl relative z-10">
          ...
          {/* Form card */}
          <div className="bg-card rounded-xl shadow-card border border-border/50 p-6 sm:p-8 fade-in" style={{
          animationDelay: '0.2s'
        }}>
            {/* Type toggle */}
            <div className="flex gap-2 p-1 bg-secondary/50 rounded-lg mb-6">
              <button onClick={() => setType('birthdate')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-md text-sm font-medium transition-all ${type === 'birthdate' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
                <Calendar className="h-4 w-4" />
                <span>Geboortedatum</span>
              </button>
              <button onClick={() => setType('range')} className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-md text-sm font-medium transition-all ${type === 'range' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}>
            ...
            {/* Main input section */}
            <div className="mb-6">
              {type === 'birthdate' ? <DateInput label={t('birthDateLabel') as string} value={birthDate} onChange={setBirthDate} error={errors.main} /> : <YearRangeInput value={yearRange} onChange={setYearRange} error={errors.main} />}
            </div>
            ...
            <div className="mt-6 pt-6 border-t border-border">
              <Button onClick={handleGenerate} className="w-full btn-vintage h-14 text-lg font-semibold text-primary-foreground rounded-lg">
          ...
          <div className="grid grid-cols-3 gap-4 mt-8 text-center">
            {[{
            icon: '🎂',
            text: 'Jouw speciale dag'
          }, {
            icon: '🌍',
            text: 'Wereldgebeurtenissen'
          }, {
            icon: '⭐',
            text: 'Beroemde jarigen'
          }].map((hint, idx) => <div key={idx} className="text-sm text-muted-foreground">
