  ...
  try {
    ...
    for (const result of results.slice(0, 3)) {
      try {
        ...
        if (blockedExtensions.some((ext) => lowerTitle.endsWith(ext))) {
          console.log(`Blocked non-image format: ${title}`);
          continue;
        }
  ...
  try {
    ...
    for (const result of results.slice(0, 3)) {
      try {
        ...
        // Verify it's actually from Nationaal Archief...
        ...
        const isFromNA = categories.some(
          (cat: { title: string }) =>
...
async function searchTMDBPerson(query: string): Promise<{ imageUrl: string; source: string } | null> {
    ...
    const searchMovies = async (): Promise<{ imageUrl: string; source: string } | null> => {
      ...
      if (year) {
        ...
        for (const { movie, yearDiff } of scored.slice(0, 5)) {
          ...
          if (yearDiff > 3) {
            console.log(
              `TMDB Movie: skipping "${movie.title}" (${movie.release_date?.substring(0, 4)}) - ${yearDiff} years from target ${year}`,
            );
            continue;
          }
          ...
          if (imagePath) {
            ...
            console.log(
              `TMDB: found movie "${movie.title}" (${movie.release_date?.substring(0, 4)}) for query "${cleanedQuery}" target year ${year}`,
            );
            return { imageUrl, source };
          }
        }
    ...
    const searchTV = async (): Promise<{ imageUrl: string; source: string } | null> => {
      ...
      if (year) {
        ...
        for (const { show, showYear, yearDiff } of scored.slice(0, 5)) {
          ...
          if (yearDiff > 2) {
            console.log(
              `TMDB TV: skipping "${show.name}" (${showYear}) - started ${yearDiff} years after target ${year}`,
            );
            continue;
          }
...
async function searchFirecrawlWikipediaOnly(
  ...
  try {
    ...
    const searchQuery = year ? `site:wikipedia.org "${query}" ${year}` : `site:wikipedia.org "${query}"`;
...
async function searchAllSources(
  ...
  const allSources = [
    ...
    trySourceWithFallback((q, y, opts) => searchWikipediaWithImages(q, y, "nl", opts), fallbackQuery, year),
    // International sources
    trySourceWithFallback((q, y, opts) => searchWikipediaWithImages(q, y, "en", opts), fallbackQuery, year),
    trySourceWithFallback((q, y, opts) => searchWikipediaWithImages(q, y, "de", opts), fallbackQuery, year),
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, images: finalResults }), {
